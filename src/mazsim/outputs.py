...
def save_output_summaries():
    output_dir = Path.joinpath(orca.get_injectable('project_dir'),'output','output_summaries')
...
def start_run_log(project_dir, run_number):
...
def get_last_run_number(project_dir):
    ...
    output_dir = Path(project_dir) / "output" / "archive"
    run_numbers = []
...
def archive_results():
    project_dir = orca.get_injectable('project_dir')
    run_number = orca.get_injectable('run_number')
    output_dir = Path.joinpath(project_dir, 'output')
    archive_dir = Path.joinpath(project_dir, 'output', 'archive')
